...
import { exchangeGoogleCode, getGmailProfile } from "@/lib/google/client";
import { getMailRedirectUri } from "@/lib/google/config";
...
export async function GET(req: Request) {
  ...
  const cookieState = req.headers
    ...
    ?.split(";")
    .map((c) => c.trim())
    .find((c) => c.startsWith("mail_oauth_state="))
    ?.split("=")[1];

  function redirectClearingCookie(pathWithQuery: string) {
    const res = NextResponse.redirect(appUrl(pathWithQuery));
    res.cookies.set("mail_oauth_state", "", { maxAge: 0, path: "/" });
    return res;
  }

  if (errorParam) {
    return redirectClearingCookie("/connections?mail=cancelled");
  }
  if (!code || !state || !cookieState || state !== cookieState) {
    return redirectClearingCookie("/connections?mail=error");
  }
  ...
  try {
    const tokens = await exchangeGoogleCode(code, getMailRedirectUri());
    ...
    if (tokens.refresh_token) {
      const { data, error } = await admin.rpc("store_oauth_secret", {
        ...
        p_name: `mail_refresh_token_${user.id}`,
      });
    ...
    if (existing) {
      await admin.from("mail_connections").update(row).eq("id", existing.id);
    } else {
      await admin.from("mail_connections").insert(row);
    }

    return redirectClearingCookie("/connections?mail=connected");
  } catch (err: any) {
    console.error("Mail OAuth callback failed:", err?.message ?? err);
    return redirectClearingCookie("/connections?mail=error");
  }
}
