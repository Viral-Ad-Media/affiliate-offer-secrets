...
import { GOOGLE_OAUTH_BASE, MAIL_SCOPES, getGoogleClientId, getMailRedirectUri, isGoogleOAuthConfigured } from "@/lib/google/config";
...
export async function GET() {
  ...
  if (!isGoogleOAuthConfigured()) {
    return NextResponse.redirect(appUrl("/connections?mail=not_configured"));
  }

  const state = crypto.randomUUID().replace(/-/g, "");
  const authUrl = new URL(GOOGLE_OAUTH_BASE);
  authUrl.searchParams.set("client_id", getGoogleClientId());
  authUrl.searchParams.set("redirect_uri", getMailRedirectUri());
  authUrl.searchParams.set("state", state);
  authUrl.searchParams.set("scope", MAIL_SCOPES.join(" "));
