...
**Only the canonical host is crawlable, and `app/robots.ts` decides that itself.** Its rules are
host-independent (`Allow: /` plus a Sitemap line from `APP_URL`), so whichever host serves them
invites indexing of the whole marketing site under THAT origin — and that happened: while
`NEXT_PUBLIC_APP_URL` still named the raw `*.netlify.app` hostname (necessarily, before the domain
was attached), the site was fully crawlable there and published a sitemap listing every marketing
page under an autogenerated Netlify name. Fixing the env var fixed it incidentally — a
non-canonical host now classifies `custom` and middleware rewrites it to `/d`, where nothing
matches (measured: `/`, `/pricing`, `/robots.txt`, `/sitemap.xml` all 404 there, with
`x-robots-tag: noindex` on the 404). **That safety was a side effect of one env var being right,
which is not where a duplicate-content control belongs**, so `robots.ts` now answers
`Disallow: /` on any host `classifyHost` doesn't call `app`, failing closed on an absent Host. It
also covers the case the rewrite does NOT: a **workspace subdomain** classifies `workspace`, is
never rewritten, and would otherwise serve the crawlable file verbatim — the same reasoning behind
the `X-Robots-Tag: noindex` middleware already sets on `/b/*` for those hosts. `www` needs nothing
here; Netlify's edge 301s it before middleware runs. Reading `headers()` makes the route dynamic
(`ƒ`, was `○`) — that is required, not a regression. Verified against a real `next start`: apex →
`Allow: /` + sitemap, `acme.` subdomain → `Disallow: /`, netlify.app → 404.

**Do NOT redirect the `*.netlify.app` hostname to the apex.** It is the obvious remedy and it would
break the engine: the four Vault cron URLs POST there, `pg_net` does not follow redirects, and a
3xx is a failed delivery — the same silent class as a Stripe webhook left registered on `www`.
